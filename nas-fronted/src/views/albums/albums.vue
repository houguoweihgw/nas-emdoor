<template>
  <div class="demo-image__preview">
    <div v-if="this.photos.length<=0">
      <el-empty description="这个相册还是空的哦，试试添加照片到相册吧"></el-empty>
    </div>
    <div class="selected-count" v-if="selectedPhotosCount > 0">已选 {{ selectedPhotosCount }} 张照片
      <el-button type="primary" @click="dialogVisible = true">批量移出相册</el-button>
      <el-dialog
          title="提示"
          :visible.sync="dialogVisible"
          width="30%">
        <span>确定从相册中批量移除照片？</span>
        <span slot="footer" class="dialog-footer">
            <el-button @click="dialogVisible = false">取 消</el-button>
            <el-button type="primary" @click="removeFromAlbum">确 定</el-button>
        </span>
      </el-dialog>
    </div>
    <el-row :gutter="2">
