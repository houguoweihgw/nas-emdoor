...
export default {
    ...
    methods: {
        ...
        fetchTotalClusterCount() {
            axios.get(`/home/clusterPhotoCount?username=${this.username}&cluster=${this.clusterName}`)
                .then(response => {
                    this.totalClusterCount = response.data.total;
        ...
        async fetchClusterPhotos(newPage) {
            try {
                this.currentPage = newPage; // 更新页码
                // console.log(this.labelName)
                //todo：将token加入请求头中进行验证
                // 发起请求，传递当前页码和每页数量
                const response = await axios.get(`/home/clusterPhotos`,
                    {
                        params: {
